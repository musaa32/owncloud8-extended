...
abstract class Flysystem extends Common {
	...
	protected function buildFlySystem(AbstractAdapter $adapter) {
	 ...
	 * @return bool
	 */
	protected function copyRecursive($fullSource, $fullTarget) {
		$content = $this->flysystem->listContents($fullSource);
		$result = false;
		foreach ($content as $item) {
			if ($item['type'] === 'dir') {
				$this->flysystem->createDir($fullTarget . '/' . $item['basename']);
				$result = $this->copyRecursive($fullSource . '/' . $item['basename'], $fullTarget . '/' . $item['basename']);
			} else {
				$result = $this->flysystem->copy($fullSource . '/' . $item['basename'], $fullTarget . '/' . $item['basename']);
			}
			if (!$result) {
				break;
			}
